...
# load a wine data
wine_path = "05.WineQuality/part_dataset.CSV"
wineq_numpy = np.loadtxt(wine_path, dtype=np.float32, delimiter=",", skiprows=1)
wineq = torch.from_numpy(wineq_numpy)

# data : residual sugar, chlorides, free sulfur dioxide, density, pH, alcohol  속성에 해당하는 데이터 -> 6
data = wineq[:, :-1]    
...
seq_model = nn.Sequential(OrderedDict([
    ('hidden1_linear', nn.Linear(6, 8)),
    ('hidden1_activation', nn.LeakyReLU()),
    ('hidden2_linear', nn.Linear(8, 16)),
    ('hidden2_activation', nn.LeakyReLU()),
    ('hidden3_linear', nn.Linear(16, 8)),
...
plt.savefig("05.WineQuality/part_wine_data_img/hidden3#8#16#8_epoch50.png", format="png")